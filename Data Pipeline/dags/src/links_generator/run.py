...
# Configuration
MAX_TIMEOUT = 30
MAX_THREADS = 1  # Number of parallel threads for crawling
VISITED_URLS = set()
BLOCKED_SITES = [
...
EXCEL_OUTPUT_FILE = "/opt/airflow/dags/src/page_scrapper/urls.xlsx"
...
def start_webdriver():
    ...
    options.add_argument("--no-sandbox")  # Bypass OS security (required in Docker)
    options.add_argument("--disable-dev-shm-usage")  # Avoid /dev/shm memory issues
    options.add_argument("--disable-gpu")  # Disable GPU acceleration (can cause crashes)
...
def save_to_excel():
    ...
    try:
        df = pd.DataFrame(reachable_urls_data, columns=['URL'])
